...
/**
 * Prints a dependency graph obtained from deps.dev for the resolved coordinates.
 *
 * Output:
 * - For each input coordinate (resolved via CoordinatesAwareTask -Pcoordinates), prints a header "[G:A:V]"
 *   and then "parent -> child" edges, first all direct edges of the root, then breadth-first transitively.
 *
 * API:
...
public class DepsDevGraphTask extends CoordinatesAwareTask {

    private static final String BASE_URL = "https://api.deps.dev/v3/systems/maven/versions/%s/dependencies";
    private static final String USER_AGENT = "graalvm-reachability-metadata/DepsDevGraphTask (+https://github.com/oracle/graalvm-reachability-metadata)";
    ...
    protected List<String> resolveCoordinates() {
        ...
        if (!filter.isEmpty() && !filter.startsWith("samples:")) {
            ...
            if (parts.length == 3 && !parts[0].isBlank() && !parts[1].isBlank() && !parts[2].isBlank()) {
                return java.util.List.of(filter);
            }
        }
    ...
    @TaskAction
    public void printDepsDevGraphs() {
        ...
        for (String coordinate : coords) {
            try {
                ...
                throw new GradleException("Failed to print deps.dev graph for " + coordinate + ": " + e.getMessage(), e);
            }
        }
    }
